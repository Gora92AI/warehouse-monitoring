...
def get_ethylene_status(ppm):
    if ppm is None: return "Unknown", "gray"
...
with col1:
    ...
    st.caption("Temperature + Humidity + Ethylene")
    ...
    if latest_station1:
        ...
        with m3:
            ...
            st.metric("🍃 Ethylene", f"{ethylene:.2f} ppm" if ethylene is not None else "N/A")
...
with g1:
    temp_f = celsius_to_fahrenheit(latest_station1.get('temperature')) if latest_station1 else 0
    st.plotly_chart(create_gauge_chart(temp_f, "Temp (S1)", 0, 100, [32, 50, 70], "°F"), use_container_width=True)
with g2:
    humidity = latest_station1.get('humidity') if latest_station1 else 0
    st.plotly_chart(create_gauge_chart(humidity, "Humidity (S1)", 0, 100, [30, 60, 80], "%"), use_container_width=True)
with g3:
    eth1 = latest_station1.get('ethylene') if latest_station1 else 0
    st.plotly_chart(create_gauge_chart(eth1, "Ethylene (S1)", 0, 15, [1, 5, 10], " ppm"), use_container_width=True)
with g4:
    eth2 = latest_station2.get('ethylene') if latest_station2 else 0
